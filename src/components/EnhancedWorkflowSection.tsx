...
const workflowSteps = [
  {
    ...
    description:
      "Simply upload bank statements in any format. CypherSOL handles complex PDFs, Excel sheets, and CSV files with ease.",
    Icon: FileText,
  ...
  },
  {
    ...
    description:
      "The system scans for inconsistencies, duplicates, and formatting issues, flagging potential problems before processing.",
    Icon: AlertTriangle,
  ...
  },
  {
    ...
    description:
      "Transactions are intelligently categorized using 7500+ data points, dramatically reducing manual sorting and suspense entries.",
    Icon: BarChart3,
  ...
  },
  {
    ...
    description:
      "Smart handling of unclear entries with suggested categories and name merging to minimize uncategorized transactions.",
    Icon: Repeat,
  ...
  },
  {
    ...
    description:
      "Seamlessly transfer organized data directly into Tally with perfect formatting, ready for accounting operations.",
    Icon: Upload,
  ...
  },
  {
    ...
    description:
      "Generate comprehensive, audit-ready reports immediately for tax filing and client presentations.",
    Icon: Download,
    ...
    <div className="container mx-auto px-4 py-16">
              ...
              {workflowSteps.map((step, index) => (
                <div key={step.id} className="relative">
                  <div className="flex flex-col md:flex-row md:items-start">
                    ...
                    <div
                      className={`relative flex items-center justify-center w-16 h-16 rounded-full bg-gradient-to-r ${step.color} shadow-lg z-10 mx-auto md:mx-0`}
                    >
                      <step.Icon className="w-8 h-8 text-white" />
                      {/* Pulsing effect */}
                      <div
                        className={`absolute w-full h-full rounded-full bg-gradient-to-r ${step.color} opacity-30 animate-ping`}
                      ...
                      <div className="mt-4 flex flex-wrap gap-2">
                        {step.id === 1 && (
                          <>
                            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-900/30 text-blue-300">
                              <Check className="w-3 h-3 mr-1" /> All formats
                              supported
                            </span>
                            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-900/30 text-blue-300">
                              <Check className="w-3 h-3 mr-1" /> Unlimited
                              uploads
                            </span>
                          </>
                        )}
                        {step.id === 2 && (
                          <>
                            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-900/30 text-purple-300">
                              <Check className="w-3 h-3 mr-1" /> Error
                              prevention
                            </span>
                            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-900/30 text-purple-300">
                        ...
                        {step.id === 4 && (
                          <>
                            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-900/30 text-yellow-300">
                              <Check className="w-3 h-3 mr-1" /> Clarity
                              improvement
                            </span>
                            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-900/30 text-yellow-300">
                        ...
                        {step.id === 5 && (
                          <>
                            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-900/30 text-red-300">
                              <Check className="w-3 h-3 mr-1" /> Seamless
                              integration
                            </span>
                            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-900/30 text-red-300">
                              <Check className="w-3 h-3 mr-1" /> Error-free
                              transfer
                            </span>
                          </>
                        )}
                        {step.id === 6 && (
                          <>
                            ...
                            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-cyan-900/30 text-cyan-300">
                              <Check className="w-3 h-3 mr-1" /> Instant
                              compliance
                            </span>
                          </>
                        )}
      ...
      <div className="bg-gradient-to-r from-blue-900/20 to-cyan-900/20 border border-blue-900/30 rounded-lg p-6 max-w-4xl mx-auto">
        <h4 className="text-lg font-medium text-blue-300 mb-2">
          Infographic Integration Note:
        </h4>
        <p className="text-gray-300">
          The final version will include an interactive SVG/visualization that
          graphically represents each step of the workflow, showing how data
          flows through the CypherSOL system from raw bank statements to
          organized, Tally-ready information.
        </p>
